 ...
 */
angular.module('hwk.dashboardModule').controller( 'hwk.dashboardController', ['$scope', '$rootScope', '$resource', '$window', '$location', '$interval', '$q', '$modal', 'hwk.dashboardService', 'hwk.filterService', 'Notifications',
  function ($scope, $rootScope, $resource, $window, $location, $interval, $q, $modal, dashboardService, filterService, Notifications) {
    'use strict';
    ...
    $scope.about = function() {
      var dashPromise = dashboardService.Status().query();
      $q.all([dashPromise.$promise]).then(function(results) {
        $scope.aboutModal.title = 'About Hawkular Alerting...';
        $scope.aboutModal.placeholder = 'About...';
        $scope.aboutModal.json = angular.toJson(results[0],true);
        $scope.aboutModal.readOnly = true;

        var modalInstance = $modal.open({
          templateUrl: 'aboutModal.html',
          backdrop: false, // keep modal up if someone clicks outside of the modal
          controller: function ($scope, $modalInstance, $log, aboutModal) {
