...
public interface MsgLogger extends BasicLogger {
    ...
    @Message(id = 240002, value = "No ActionPluginListener found on plugin deployment")
    void warnNoPluginsFound();

    @LogMessage(level = Logger.Level.INFO)
    @Message(id = 240003, value = "Plugin [%s] has sent a registration request: [%s]")
    void infoPluginRegistration(String actionPlugin, String msg);

    @LogMessage(level = Logger.Level.ERROR)
    @Message(id = 240005, value = "Plugin cannot send a message to the bus. Error: [%s]")
    void errorCannotSendMessage(String msg);

    @LogMessage(level = Logger.Level.WARN)
    @Message(id = 240007, value = "Plugin received a message without plugin info.")
    void warnMessageReceivedWithoutPluginInfo();

    @LogMessage(level = Logger.Level.WARN)
    @Message(id = 240008, value = "Cannot connect to the broker. Attempt [%s]. Trying in [%s] ms. Error: [%s]")
