...
def resp = client.get(path: "events", query: [categories: "DEPLOYMENT"])
if (resp.status == 200) {
    resp.data.each {
        println "${it}"
    }
...
println "\nQuery LOG events:\n"
resp = client.get(path: "events", query: [categories: "LOG"])
if (resp.status == 200) {
    resp.data.each {
        println "${it}"
    }
...
println "\nQuery appA,appB events:\n"
resp = client.get(path: "events", query: [tags: "app|appA,app|appB"])
if (resp.status == 200) {
    resp.data.each {
        println "${it}"
    }
